...
def get_FanList(user_info, driver):
    # 获取想要研究的微博名
...
def crawlFanList(url_fanList, user_info, driver):
    ...
    for page in range(pagination):
        ...
        for fan in fans:
            #try:
            ucid0 = fan.find_element_by_css_selector('div > div > div > div.follow_box > div.follow_inner > ul > li > dl > dt > a > img').get_attribute('usercard')
            ucid = get_ID(ucid0)
            ...
            #开始爬取并放入到数据库中
            crawlFollowerList(ucid, user_follower, user_info, driver)
...
#该方法爬取粉丝关注的所有账户
def crawlFollowerList(follower, user_follower, user_info, driver):
    ...
    for page in range(5):#5
        ...
        for account in accounts:
            ...
            #name = account.find_element_by_css_selector('dl>dt>a').get_attribute('title')
            ucid0 = account.find_element_by_css_selector('dl>dt>a>img').get_attribute('usercard')

            try:
                ucid = get_ID(ucid0)
                data_userFollower = (ucid, follower)
                user_follower.createUserFollower(data_userFollower)
...
if __name__ == "__main__":
    ...
    url_fanList = get_FanList(user_info, driver)
