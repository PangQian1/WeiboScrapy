...
def searchGoalAccount(user_info, driver):
    ...
    while(True):
        ...
        if (verify_tag == 'Y' or verify_tag == 'y'):
            ...
            # 在输入框中输入搜索内容并进行搜索
            driver.find_element_by_css_selector('#pl_common_searchTop > div.search_head.clearfix > div.search_head_formbox > div.search_input > div > div.searchInp_box > div > input').clear()
            driver.find_element_by_css_selector('#pl_common_searchTop > div.search_head.clearfix > div.search_head_formbox > div.search_input > div > div.searchInp_box > div > input').send_keys(goal_weibo)
            driver.find_element_by_css_selector('#pl_common_searchTop > div.search_head.clearfix > div.search_head_formbox > div.search_input > div > div.searchBtn_box > div > a').click()
